...
function Feed() {
	...
	return (
		<div className="mb-8 flex w-full grow flex-col items-center justify-center">
			<div className="flex w-full max-w-3xl grow flex-col items-center justify-start rounded-lg bg-gray-100 py-8">
				<div className="mb-8 flex w-full items-center justify-between px-10">
					...
					{media.length == 0 ? (
						...
						<button
							...
							type="button"
							disabled={chosenMedia.length === 0}
							className="relative mr-2 items-center rounded-lg bg-primary-700 px-5 py-2.5 text-center text-sm font-medium text-white hover:bg-primary-800 focus:outline-none focus:ring-4 focus:ring-[#24292F]/50 dark:hover:bg-[#050708]/30 dark:focus:ring-gray-500"
						>
							{chosenMedia.length === 0 && (
								<div className="absolute left-0 top-0 z-10 h-full w-full rounded-lg bg-gray-50/50" />
							)}
							Importar posts
						</button>
					)}
				...
				<div className="grid h-full min-h-fit w-full max-w-2xl grid-cols-3 gap-2">
					{media &&
						media.map((e) => (
							<button
								key={e.id}
